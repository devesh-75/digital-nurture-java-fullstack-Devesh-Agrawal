-- ============================================================
-- Country reference data (used by Hands on 1 - Query Methods)
-- ============================================================
delete from country;
...
-- ============================================================
-- Stock sample data (used by Hands on 2 - Query Methods)
-- NOTE: this is a small anchor subset (not the full one-year
-- dataset from stock-data.csv) containing exactly the rows
-- needed to reproduce the expected results in the exercise.
-- Load the full generated stock-data.sql instead if you have it.
-- ============================================================
-- Facebook (FB) - September 2019 (Hands on 2, query 1)
...
-- Google (GOOGL) - dates where closing price was above 1250 (Hands on 2, query 2)
...
-- Facebook (FB) - the three highest-volume trading days (Hands on 2, query 3)
...
-- Netflix (NFLX) - the three lowest-closing-price days (Hands on 2, query 4)
...
-- ============================================================
-- Payroll seed data (used by Hands on 3-6 - O/R Mapping)
-- ============================================================
-- Departments
...
-- Employees (em_dp_id 1=Engineering, 2=Sales, 3=Human Resources)
-- department 1 (Engineering) intentionally has two employees (ids 1 and 2)
-- to demonstrate the one-to-many relationship in Hands on 5
insert into employee (em_name, em_salary, em_permanent, em_date_of_birth, em_dp_id) values ('John Doe', 75000.00, true, '1990-05-15', 1);
insert into employee (em_name, em_salary, em_permanent, em_date_of_birth, em_dp_id) values ('Jane Smith', 65000.00, true, '1988-03-22', 1);
insert into employee (em_name, em_salary, em_permanent, em_date_of_birth, em_dp_id) values ('Mike Brown', 55000.00, false, '1995-07-10', 2);
insert into employee (em_name, em_salary, em_permanent, em_date_of_birth, em_dp_id) values ('Emma Davis', 60000.00, true, '1992-11-30', 2);
insert into employee (em_name, em_salary, em_permanent, em_date_of_birth, em_dp_id) values ('Chris Wilson', 50000.00, true, '1985-01-05', 3);

-- Employee <-> Skill (many-to-many). Employee 4 (Emma Davis) intentionally
-- has no skill yet, so Hands on 6 can add skill id 4 (Leadership) to her.
